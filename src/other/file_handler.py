...
class Pdf2Img_convertor:
    ...
    def pdf_2_jpg(self):
        ...
        pdf_list = [self.input_pdf_path + '/'+f 
                     for f in os.listdir(self.input_pdf_path) 
                     if os.path.isfile(os.path.join(self.input_pdf_path, f))]
        
        print("Number of PDF="+str(len(pdf_list)))
        
        # Counting the number of pages in each pdf and converting the pages into image
        for file in pdf_list:
            images = convert_from_path(file, 500,poppler_path=r'C:\Program Files\poppler-0.68.0\bin')
            i = 1
            length = len(images)
        ...
        for image in images:
            image.save(self.output_img_path + str(i) + '.jpg', 'Jpeg')
            i = i + 1
    ...
    def pdf_2_png(self):
        ...
        for file in pdf_list:
            ...
            for image in images:
                print('hello')
                image.save(self.output_img_path + str(i) + '.png', 'png')
                i = i + 1
